  ...
  <div class="container">
    ...
    <van-cell-group title="服务需求" class="cell">
      <van-field
        ...
        style="height: 100px"
        v-model="form.demand"
        placeholder="请简要描述您要就诊的科室"
      />
    </van-cell-group>

    <van-button @click="sumbit" class="subm it" type="primary" size="large">
      提交订单
    </van-button>
...
const showHospitalColumn = computed(() => {
...
const showHospitalOnConfirm = (item) => {
  console.log(item, "showHospitalOnConfirm");
...
const showTimeConfirm = (item) => {
  // 查看选择日期后获取的数据
  console.log(item, "dateInfo");
  // 后端要求starttime是number类型，而获取的数据为一个数组
  // 将selectedValues数组数据组装成时间格式，再转换为时间戳
...
const showComponionComfirm = (item) => {
  console.log(item, "showComponionComfirm");
